...
import { makeObject, hasTexture, type Orientation, type VariantKey } from "./assetManifest";
...
export class WorldRenderer {
  focusPoint(sessionId: string): {x:number;y:number}|null {
    const room = this.views.get(sessionId);
    return room ? {x:this.scene.x+room.orch.x,y:this.scene.y+room.orch.y} : null;
  }
  ...
  private groundTexture() {
    ...
    ctx.fillStyle = "#263443"; ctx.fillRect(0,0,128,128);
