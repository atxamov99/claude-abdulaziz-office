...
export class Stage {
  ...
  setContentSize(w: number, h: number): void {
    const changed = this.contentWidth !== w || this.contentHeight !== h;
    this.contentWidth = w;
    this.contentHeight = h;
    if (this.autoFit && changed && w > 0 && h > 0) this.fitContent();
  }

  fitContent(): void {
    if (!this.contentWidth || !this.contentHeight) return;
    this.setZoom(Math.min(1, (this.app.screen.width - 40) / this.contentWidth, (this.app.screen.height - 40) / this.contentHeight));
    this.setWorld((this.app.screen.width - this.contentWidth * this.world.scale.x) / 2, (this.app.screen.height - this.contentHeight * this.world.scale.x) / 2);
  }
