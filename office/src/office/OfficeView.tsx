 ...
 */
export function OfficeView({focusSessionId}:{focusSessionId?:string} = {}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const stageRef = useRef<Stage | null>(null);
  const rendererRef=useRef<WorldRenderer|null>(null);
  const focusRef=useRef(focusSessionId);
  focusRef.current=focusSessionId;
  const focusRoom=()=>{
    const stage=stageRef.current;
    const point=focusRef.current?rendererRef.current?.focusPoint(focusRef.current):null;
    if(!stage || !point) return;
    stage.autoFit=false;
    stage.setZoom(0.85);
    stage.setWorld(stage.app.screen.width/2-point.x*0.85,stage.app.screen.height/2-point.y*0.85);
  };
  useEffect(()=>{focusRoom();},[focusSessionId]);
  ...
  return (
    <div className="office-wrap">
      ...
      <button style={{position:"absolute",left:16,top:16,zIndex:2}} onClick={()=>stageRef.current?.fitContent()}>Весь офис</button>
